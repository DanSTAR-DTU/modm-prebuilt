...
#ifndef MODM_STM32_TIMER_16_HPP
#define MODM_STM32_TIMER_16_HPP
 ...
 */
class Timer16 : public GeneralPurposeTimer
{
public:
	...
	connect()
	{
		using Connector = GpioConnector<Peripheral::Tim16, Signals...>;
		Connector::connect();
	}
	...
	setPeriod(uint32_t microseconds, bool autoApply = true)
	{
		...
		// unequal to zero)
		uint32_t cycles = microseconds * (SystemClock::Timer16 / 1'000'000UL);
	...
	getTickFrequency()
	{
		return SystemClock::Timer16 / (TIM16->PSC + 1);
	}
...
public:
	...
	setInvertedPwm(uint32_t channel)
	{
...

		{
			...
			if (channel <= 1)
			{
				...
				flags <<= offset;
				flags |= TIM16->CCMR1 & ~(TIM_CCMR1_OC1M << offset);
				TIM16->CCMR1 = flags;
			}
			else {
				...
				flags <<= offset;
				flags |= TIM16->CCMR2 & ~(TIM_CCMR1_OC1M << offset);
	...
	{
		*(&TIM16->CCR1 + (channel - 1)) = value;
	}

	static inline Value
	getCompareValue(uint32_t channel)
	{
		return *(&TIM16->CCR1 + (channel - 1));
	}
...
public:
	...
	{
		return InterruptFlag_t(TIM16->DIER);
	}

	static inline void
	enableDmaRequest(DmaRequestEnable dmaRequests)
	{
		TIM16->DIER |= static_cast<uint32_t>(dmaRequests);
	}
	...
	getInterruptFlags()
	{
		return InterruptFlag_t(TIM16->SR);
	}
