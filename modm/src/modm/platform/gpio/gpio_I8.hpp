...
public:
	...
	struct Tamp2 { static void connect();
		static_assert(
			...
			"GpioI8::Tamp2 only connects to Rtc!");
	};
	template< Peripheral peripheral >
	struct Ts { static void connect();
		static_assert(
			(peripheral == Peripheral::Rtc),
			"GpioI8::Ts only connects to Rtc!");
	};
	template< Peripheral peripheral >
	struct Wkup5 { static void connect();
		static_assert(
			(peripheral == Peripheral::Sys),
			"GpioI8::Wkup5 only connects to Sys!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
template<>
struct GpioI8::Tamp2<Peripheral::Rtc>
{
	using Gpio = GpioI8;
...
template<>
struct GpioI8::Ts<Peripheral::Rtc>
{
	using Gpio = GpioI8;
...
template<>
struct GpioI8::Wkup5<Peripheral::Sys>
{
	using Gpio = GpioI8;
