...
#include "spi_master_6.hpp"

// Bit0: single transfer state
// Bit1: block transfer state
uint8_t
modm::platform::SpiMaster6::state(0);

uint8_t
modm::platform::SpiMaster6::count(0);

void *
modm::platform::SpiMaster6::context(nullptr);

modm::Spi::ConfigurationHandler
modm::platform::SpiMaster6::configuration(nullptr);
// ----------------------------------------------------------------------------

uint8_t
modm::platform::SpiMaster6::acquire(void *ctx, ConfigurationHandler handler)
{
	if (context == nullptr)
	{
		context = ctx;
...
uint8_t
modm::platform::SpiMaster6::release(void *ctx)
{
	if (ctx == context)
	{
		if (--count == 0)
...
modm::platform::SpiMaster6::transfer(uint8_t data)
{
	...
	if ( !(state & Bit0) )
	{
		...
		if (!SpiHal6::isTransmitRegisterEmpty())
	...
	if (!SpiHal6::isReceiveRegisterNotEmpty())
		return {modm::rf::Running};
...
modm::platform::SpiMaster6::transfer(
