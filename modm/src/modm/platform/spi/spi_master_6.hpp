...
#ifndef MODM_STM32_SPI_MASTER6_HPP
#define MODM_STM32_SPI_MASTER6_HPP

#include <modm/architecture/interface/spi_master.hpp>
#include <modm/platform/gpio/connector.hpp>
#include "spi_hal_6.hpp"
...
/**
 * Serial peripheral interface (SPI6).
 *
 * Simple unbuffered implementation.
 *
 * @author	Niklas Hauser
 * @ingroup	modm_platform_spi modm_platform_spi_6
 */
class SpiMaster6 : public modm::SpiMaster
{
...
public:
	...
	connect()
	{
		using Connector = GpioConnector<Peripheral::Spi6, Signals...>;
	...
	initialize()
	{
		...
		constexpr float desired = float(SystemClock::Spi6) / baudrate;
		...
		assertBaudrateInTolerance<
				SystemClock::Spi6 / nearest,
				baudrate,
				tolerance >();

		// translate the prescaler into the bitmapping
		constexpr SpiHal6::Prescaler prescaler = (
				(nearest >= 256) ? SpiHal6::Prescaler::Div256 : (
				(nearest >= 128) ? SpiHal6::Prescaler::Div128 : (
				(nearest >=  64) ? SpiHal6::Prescaler::Div64  : (
				(nearest >=  32) ? SpiHal6::Prescaler::Div32  : (
				(nearest >=  16) ? SpiHal6::Prescaler::Div16  : (
				(nearest >=   8) ? SpiHal6::Prescaler::Div8   : (
				(nearest >=   4) ? SpiHal6::Prescaler::Div4   :
								   SpiHal6::Prescaler::Div2
				)))))));

		// initialize the Spi
		SpiHal6::initialize(prescaler);
		state = 0;
	}
	...
	{
		SpiHal6::setDataMode(static_cast<SpiHal6::DataMode>(mode));
	}

	static modm_always_inline void
	setDataOrder(DataOrder order)
	{
		SpiHal6::setDataOrder(static_cast<SpiHal6::DataOrder>(order));
	}
...
#endif // MODM_STM32_SPI_MASTER6_HPP