...
modm::platform::Fdcan3::Mode modm::platform::Fdcan3::last_set_mode = modm::platform::Fdcan3::Mode::Normal;
...
void modm::platform::Fdcan3::printRegs()
{
	MODM_LOG_INFO << "\nREGS can 2!!!!\n";
	MODM_LOG_INFO << "ECR: " << (uint32_t)(FDCAN3->ECR) << modm::endl;
	MODM_LOG_INFO << "PSR: " << (uint32_t)(FDCAN3->PSR) << modm::endl;
	MODM_LOG_INFO << "TXBTO: " << (uint32_t)(FDCAN3->TXBTO) << modm::endl;
	MODM_LOG_INFO << "CCCR: " << (uint32_t)(FDCAN3->CCCR) << modm::endl;
}

bool modm::platform::Fdcan3::enableProtectedAccess()
{
	FDCAN3->CCCR = FDCAN_CCCR_INIT;
	int deadlockPreventer = 10'000;
	while (((FDCAN3->CCCR & FDCAN_CCCR_INIT) == 0) and (deadlockPreventer-- > 0)) {
		modm::delay_us(1);
...
void modm::platform::Fdcan3::disableProtectedAccess()
{
	FDCAN3->CCCR &= ~FDCAN_CCCR_INIT;
}
...
{
	if(!Rcc::isEnabled<Peripheral::Fdcan3>()) {
		RCC->CCIPR |= RCC_CCIPR_FDCANSEL_1;
		Rcc::enable<Peripheral::Fdcan3>();
	}
	...
	FDCAN3->IE = FDCAN_IE_RF0FE | FDCAN_IE_RF0NE | FDCAN_IE_RF1NE | FDCAN_IE_RF1FE
				| FDCAN_IE_TCE | FDCAN_IE_TFEE; // transmit

	// Write on interrupt line 1 and read on iterrupt line 0
	FDCAN3->ILS = FDCAN_ILS_SMSG | FDCAN_ILS_TFERR;

	FDCAN3->CCCR |= FDCAN_CCCR_FDOE | FDCAN_CCCR_BRSE; //FD mode allowed


	FDCAN3->RXGFC = overwriteOnOverrun ? 0 : (FDCAN_RXGFC_F1OM | FDCAN_RXGFC_F0OM); // TODO: or inverted?

	// Reject frames not matching any filter
	FDCAN3->RXGFC |= FDCAN_RXGFC_ANFE | FDCAN_RXGFC_ANFS;
...
sendMsg(const modm::can::LongMessage& message)
{
	...
	uint32_t* msgRam = (uint32_t*)(Fdcan3::TxFifo + (putIndex * Fdcan3::TxFifoElementSize));
	...
	*msgRam++ =  ((message.isFlexibleData() ? 1 : 0) << 21) 
	...
	FDCAN3->TXBAR = ((uint32_t)1 << putIndex);
	return true;
}
...
readMsg(modm::can::LongMessage& message, uint8_t* filter_id, uint16_t *timestamp, bool fifo0)
{
	...
	uint32_t* msgRam;
	if(fifo0) {
		getIndex = (FDCAN3->RXF0S & FDCAN_RXF0S_F0GI_Msk) >> FDCAN_RXF0S_F0GI_Pos;
		msgRam = (uint32_t*)(Fdcan3::RxFifo0 + (getIndex * Fdcan3::RxFifoElementSize));
	}
	else {
		getIndex = (FDCAN3->RXF1S & FDCAN_RXF1S_F1GI_Msk) >> FDCAN_RXF1S_F1GI_Pos;
		msgRam = (uint32_t*)(Fdcan3::RxFifo1 + (getIndex * Fdcan3::RxFifoElementSize));
	}
...
MODM_ISR(FDCAN3_IT1)
{
	...
	if (ir & FDCAN_IR_TC) { // Transmission completed
		FDCAN3->IR = FDCAN_IR_TC;
		free_spots = 1;
	}

	if (ir & FDCAN_IR_TFE) // Transmission Fifo empty
	{
		FDCAN3->IR = FDCAN_IR_TFE;
		free_spots = modm::platform::Fdcan3::TxFifoSize;
	}
...
MODM_ISR(FDCAN3_IT0)
{
	...
	{
		// release overrun flag & access the next message
		FDCAN3->IR = FDCAN_IE_RF1FE;
		fifo0 = false;
	}

	if(ir & FDCAN_IE_RF0NE) //Fifo0 receive
	{
		FDCAN3->IR = FDCAN_IE_RF0NE;
		fifo0 = true;
	}
...
modm::platform::Fdcan3::setMode(Mode mode)
{
	...
	FDCAN3->CCCR |= FDCAN_CCCR_CCE | FDCAN_CCCR_TEST;
...
void modm::platform::Fdcan3::setFilter(uint16_t sfid1, uint16_t sfid2, StandardFilterMode mode, FilterConfig config, uint8_t index)
{
	if(index < Fdcan3::FilterListStandardSize)
	{
		modm_assert_continue_ignore(enableProtectedAccess(), "can.setFilter",
			"deadlockPreventer timed out", 1);

		uint32_t* base = (uint32_t*)(Fdcan3::FilterListStandard + (index * Fdcan3::FilterListStandardElementSize));
...
void modm::platform::Fdcan3::setExtendedFilter(uint32_t efid1, uint32_t efid2, ExtendedFilterMode mode, FilterConfig config, uint8_t index)
{
	if(index < Fdcan3::FilterListExtendedSize)
	{
		modm_assert_continue_ignore(enableProtectedAccess(), "can.setExtendedFilter",
			"deadlockPreventer timed out", 1);
		
		uint32_t* base = (uint32_t*)(Fdcan3::FilterListExtended + (index * Fdcan3::FilterListExtendedElementSize));
...
void
modm::platform::Fdcan3::_setMode(Mode mode)
{
...
modm::platform::Fdcan3::sendMessage(const can::IMessage& message)
{
	...
	if ((FDCAN3->TXFQS & FDCAN_TXFQS_TFQF) != 0)
	{
...
void modm::platform::Fdcan3::setNumberOfFilters(uint8_t n)
{
	modm_assert_continue_ignore(enableProtectedAccess(), "can.setNumberOfFilters",
		"deadlockPreventer timed out", 1);
	
	FDCAN3->RXGFC |= (n << FDCAN_RXGFC_LSS_Pos) & FDCAN_RXGFC_LSS_Msk;
...
void modm::platform::Fdcan3::setNumberOfExtendedFilters(uint8_t n)
{

	modm_assert_continue_ignore(enableProtectedAccess(), "can.setNumberOfExtendedFilters",
		"deadlockPreventer timed out", 1);
	
	FDCAN3->RXGFC |= (n << FDCAN_RXGFC_LSE_Pos) & FDCAN_RXGFC_LSE_Msk;
...
void
modm::platform::Fdcan3::enableStatusChangeInterrupt(uint32_t interruptEnable, uint32_t interruptPriority)
{
	// TODO
}
