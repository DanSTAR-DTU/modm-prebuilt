...
modm::platform::Fdcan2::Mode modm::platform::Fdcan2::last_set_mode = modm::platform::Fdcan2::Mode::Normal;
...
void modm::platform::Fdcan2::printRegs()
{
	MODM_LOG_INFO << "\nREGS can 2!!!!\n";
	MODM_LOG_INFO << "ECR: " << (uint32_t)(FDCAN2->ECR) << modm::endl;
	MODM_LOG_INFO << "PSR: " << (uint32_t)(FDCAN2->PSR) << modm::endl;
	MODM_LOG_INFO << "TXBTO: " << (uint32_t)(FDCAN2->TXBTO) << modm::endl;
	MODM_LOG_INFO << "CCCR: " << (uint32_t)(FDCAN2->CCCR) << modm::endl;
}

bool modm::platform::Fdcan2::enableProtectedAccess()
{
	FDCAN2->CCCR = FDCAN_CCCR_INIT;
	int deadlockPreventer = 10'000;
	while (((FDCAN2->CCCR & FDCAN_CCCR_INIT) == 0) and (deadlockPreventer-- > 0)) {
		modm::delay_us(1);
...
void modm::platform::Fdcan2::disableProtectedAccess()
{
	FDCAN2->CCCR &= ~FDCAN_CCCR_INIT;
}
...
{
	if(!Rcc::isEnabled<Peripheral::Fdcan2>()) {
		RCC->CCIPR |= RCC_CCIPR_FDCANSEL_1;
		Rcc::enable<Peripheral::Fdcan2>();
	}
	...
	FDCAN2->IE = FDCAN_IE_RF0FE | FDCAN_IE_RF0NE | FDCAN_IE_RF1NE | FDCAN_IE_RF1FE
				| FDCAN_IE_TCE | FDCAN_IE_TFEE; // transmit

	// Write on interrupt line 1 and read on iterrupt line 0
	FDCAN2->ILS = FDCAN_ILS_SMSG | FDCAN_ILS_TFERR;

	FDCAN2->CCCR |= FDCAN_CCCR_FDOE | FDCAN_CCCR_BRSE; //FD mode allowed


	FDCAN2->RXGFC = overwriteOnOverrun ? 0 : (FDCAN_RXGFC_F1OM | FDCAN_RXGFC_F0OM); // TODO: or inverted?

	// Reject frames not matching any filter
	FDCAN2->RXGFC |= FDCAN_RXGFC_ANFE | FDCAN_RXGFC_ANFS;
...
sendMsg(const modm::can::LongMessage& message)
{
	...
	uint8_t putIndex = (FDCAN2->TXFQS & FDCAN_TXFQS_TFQPI_Msk) >> FDCAN_TXFQS_TFQPI_Pos;


	uint32_t* msgRam = (uint32_t*)(Fdcan2::TxFifo + (putIndex * Fdcan2::TxFifoElementSize));
...
readMsg(modm::can::LongMessage& message, uint8_t* filter_id, uint16_t *timestamp, bool fifo0)
{
	...
	uint32_t* msgRam;
	if(fifo0) {
		getIndex = (FDCAN2->RXF0S & FDCAN_RXF0S_F0GI_Msk) >> FDCAN_RXF0S_F0GI_Pos;
		msgRam = (uint32_t*)(Fdcan2::RxFifo0 + (getIndex * Fdcan2::RxFifoElementSize));
	}
	else {
		getIndex = (FDCAN2->RXF1S & FDCAN_RXF1S_F1GI_Msk) >> FDCAN_RXF1S_F1GI_Pos;
		msgRam = (uint32_t*)(Fdcan2::RxFifo1 + (getIndex * Fdcan2::RxFifoElementSize));
	}
...
MODM_ISR(FDCAN2_IT1)
{
	...
	if (ir & FDCAN_IR_TC) { // Transmission completed
		FDCAN2->IR = FDCAN_IR_TC;
		free_spots = 1;
	}

	if (ir & FDCAN_IR_TFE) // Transmission Fifo empty
	{
		FDCAN2->IR = FDCAN_IR_TFE;
		free_spots = modm::platform::Fdcan2::TxFifoSize;
	}
...
MODM_ISR(FDCAN2_IT0)
{
	...
	{
		// release overrun flag & access the next message
		FDCAN2->IR = FDCAN_IE_RF1FE;
		fifo0 = false;
	}

	if(ir & FDCAN_IE_RF0NE) //Fifo0 receive
	{
		FDCAN2->IR = FDCAN_IE_RF0NE;
		fifo0 = true;
	}
...
modm::platform::Fdcan2::setMode(Mode mode)
{
	...
	FDCAN2->CCCR |= FDCAN_CCCR_CCE | FDCAN_CCCR_TEST;
...
void modm::platform::Fdcan2::setFilter(uint16_t sfid1, uint16_t sfid2, StandardFilterMode mode, FilterConfig config, uint8_t index)
{
	if(index < Fdcan2::FilterListStandardSize)
	{
		modm_assert_continue_ignore(enableProtectedAccess(), "can.setFilter",
			"deadlockPreventer timed out", 1);

		uint32_t* base = (uint32_t*)(Fdcan2::FilterListStandard + (index * Fdcan2::FilterListStandardElementSize));
...
void modm::platform::Fdcan2::setExtendedFilter(uint32_t efid1, uint32_t efid2, ExtendedFilterMode mode, FilterConfig config, uint8_t index)
{
	if(index < Fdcan2::FilterListExtendedSize)
	{
		modm_assert_continue_ignore(enableProtectedAccess(), "can.setExtendedFilter",
			"deadlockPreventer timed out", 1);
		
		uint32_t* base = (uint32_t*)(Fdcan2::FilterListExtended + (index * Fdcan2::FilterListExtendedElementSize));
...
void
modm::platform::Fdcan2::_setMode(Mode mode)
{
...
void modm::platform::Fdcan2::setNumberOfFilters(uint8_t n)
{
	modm_assert_continue_ignore(enableProtectedAccess(), "can.setNumberOfFilters",
		"deadlockPreventer timed out", 1);
	
	FDCAN2->RXGFC |= (n << FDCAN_RXGFC_LSS_Pos) & FDCAN_RXGFC_LSS_Msk;
...
void modm::platform::Fdcan2::setNumberOfExtendedFilters(uint8_t n)
{

	modm_assert_continue_ignore(enableProtectedAccess(), "can.setNumberOfExtendedFilters",
		"deadlockPreventer timed out", 1);
	
	FDCAN2->RXGFC |= (n << FDCAN_RXGFC_LSE_Pos) & FDCAN_RXGFC_LSE_Msk;
...
void
modm::platform::Fdcan2::enableStatusChangeInterrupt(uint32_t interruptEnable, uint32_t interruptPriority)
{
	// TODO
}
